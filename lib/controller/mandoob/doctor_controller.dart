...
class ReserveController extends GetxController with StateMixin {
  ...
  void openPaymentUrl({required String id}) {
    Get.put(
        PaymentController('https://test.zaincash.iq/transaction/pay?id=$id'));
    ...
    DioHelper()
        ...
        .then(
            (resp) => {
                  ...
                  if (resp.data.toString() != 'You dont have enough money' &&
                      resp.data.toString() !=
                          'the doctor appointment is full for this day')
                    {
                      DioHelper()

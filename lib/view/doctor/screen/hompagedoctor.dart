import 'package:flutter/material.dart';
import 'package:flutter_svg/svg.dart';
import 'package:get/get.dart';
import 'package:mandoob/class/constant/images.dart';

import 'package:flutter_switch/flutter_switch.dart';
import 'package:mandoob/controller/doctor/homepagedoctorcontroller.dart';

import '../../../class/constant/colors.dart';
import '../widget/customlistcompanies.dart';
import 'finalinterviews.dart';

class HomePageDoctor extends GetView<HomePageDoctorController> {
  const HomePageDoctor({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
     Get.put(HomePageDoctorController());
    return SafeArea(
      child: Scaffold(
          appBar: AppBar(
              backgroundColor: Colors.white,
              leading: InkWell(
                onTap: (){
                  Get.to(()=> const FinalInterviewsScreen());
                },
                child: Padding(
                  padding: const EdgeInsets.all(13.0),
                  child: SvgPicture.asset(
                    AppImages.interviewIcon,
                    width: 20,
                    height: 20,
                    fit: BoxFit.contain,
                  ),
                ),
              ),
            title: Text(
              'Companies'.tr,
              style: const TextStyle(
                  color: Colors.black,
                  fontWeight: FontWeight.bold,
                  fontSize: 20),
            ),
            centerTitle: true,
          ),
          body: Padding(
            padding: const EdgeInsets.symmetric(vertical: 24.0),
            child: Column(
              children:const  [
                // Padding(
                //   padding: const EdgeInsets.symmetric(vertical: 16.0, horizontal: 16),
                //   child: Row(
                //     mainAxisAlignment: MainAxisAlignment.start,
                //     children: [
                //       Obx(()=> FlutterSwitch(
                //         activeColor: AppColor.secondaryColor,
                //         width: 75.0,
                //         height: 30.0,
                //         valueFontSize: 12.0,
                //         toggleSize: 18.0,
                //         value: controller.statusAvailable.value,
                //         borderRadius: 20.0,
                //         padding: 8.0,
                //         showOnOff: true,
                //         onToggle: (val) {
                //           controller.setSelect(val);
                //           print( "++++++"+val.toString());
                //           controller.changeAvailable();
                //         },
                //       ),),
                //       Obx(()=>
                //           Padding(
                //             padding: const EdgeInsets.symmetric(horizontal: 8.0),
                //             child: Text(
                //               controller.textAvailable.toString(),
                //               style: const TextStyle(
                //                   color: Colors.black,
                //                   fontSize: 16
                //               ),
                //             ),
                //           ),
                //       ),
                //
                //
                //     ],
                //   ),
                // ),
                // const SizedBox(
                //   height: 10,
                // ),
                 CustomListCompanies(),
              ],
            ),
          )
      ),
    );
  }
}
